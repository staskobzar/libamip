/* Generated by re2c 0.16 on Sat Feb 25 16:27:49 2017 */
#line 1 "parse_pack.re"
/**
 * libamip -- Library with functions for read/create AMI packets
 * Copyright (C) 2016, Stas Kobzar <staskobzar@modulis.ca>
 *
 * This file is part of libamip.
 *
 * libamip is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * libamip is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with libamip.  If not, see <http://www.gnu.org/licenses/>.
 */

/**
 * @file amip_parse.c
 * @brief AMI (Asterisk Management Interface) packet parser.
 *
 * @author Stas Kobzar <stas.kobzar@modulis.ca>
 */

#include <stdio.h>
#include <string.h>
#include "amip.h"

#define SET_HEADER(flag)  len = cur - tok; \
                          hdr_type = flag; \
                          goto yyc_key;

#define CMD_HEADER(offset, flag) len = cur - tok - offset; tok += offset; \
                          while(*tok == ' ') { tok++; len--; } \
                          len -= 2; \
                          char *val = substr(tok, len, 0); \
                          amipack_append (pack, flag, val); \
                          free (val); tok = cur; goto yyc_command;

// introducing types:re2c for prompt packet

enum yycond_pack {
  yyckey,
  yycvalue,
  yyccommand,
};

AMIPacket *amiparse_pack (const char *pack_str)
{
  AMIPacket *pack = amipack_init ();
  enum header_type hdr_type;
  const char *marker = pack_str;
  const char *cur    = marker;
  const char *ctxmarker;
  int c = yyckey;
  int len = 0;

  const char *tok = marker;
  char *hdr_name;


#line 68 "parse_pack.c"
{
	unsigned char yych;
	unsigned int yyaccept = 0;
	switch (c) {
	case yyckey: goto yyc_key;
	case yycvalue: goto yyc_value;
	case yyccommand: goto yyc_command;
	}
/* *********************************** */
yyc_command:
	yych = *cur;
	switch (yych) {
	case '\n':	goto yy5;
	case '-':	goto yy7;
	case 'A':
	case 'a':	goto yy8;
	case 'M':
	case 'm':	goto yy9;
	case 'O':
	case 'o':	goto yy10;
	case 'P':
	case 'p':	goto yy11;
	default:	goto yy3;
	}
yy3:
	yyaccept = 0;
	yych = *(marker = ++cur);
	goto yy13;
yy4:
#line 223 "parse_pack.re"
	{
              if (hdr_name) free (hdr_name);
              amipack_destroy (pack);
              return NULL;
            }
#line 104 "parse_pack.c"
yy5:
	++cur;
yy6:
#line 434 "parse_pack.re"
	{ goto yyc_command; }
#line 110 "parse_pack.c"
yy7:
	yyaccept = 0;
	yych = *(marker = ++cur);
	switch (yych) {
	case '-':	goto yy14;
	default:	goto yy13;
	}
yy8:
	yyaccept = 0;
	yych = *(marker = ++cur);
	switch (yych) {
	case 'C':
	case 'c':	goto yy15;
	default:	goto yy13;
	}
yy9:
	yyaccept = 0;
	yych = *(marker = ++cur);
	switch (yych) {
	case 'E':
	case 'e':	goto yy16;
	default:	goto yy13;
	}
yy10:
	yyaccept = 0;
	yych = *(marker = ++cur);
	switch (yych) {
	case 'U':
	case 'u':	goto yy17;
	default:	goto yy13;
	}
yy11:
	yyaccept = 0;
	yych = *(marker = ++cur);
	switch (yych) {
	case 'R':
	case 'r':	goto yy18;
	default:	goto yy13;
	}
yy12:
	++cur;
	yych = *cur;
yy13:
	switch (yych) {
	case '\n':	goto yy5;
	default:	goto yy12;
	}
yy14:
	yych = *++cur;
	switch (yych) {
	case 'E':	goto yy19;
	default:	goto yy13;
	}
yy15:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy20;
	default:	goto yy13;
	}
yy16:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy21;
	default:	goto yy13;
	}
yy17:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy22;
	default:	goto yy13;
	}
yy18:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy23;
	default:	goto yy13;
	}
yy19:
	yych = *++cur;
	switch (yych) {
	case 'N':	goto yy24;
	default:	goto yy13;
	}
yy20:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy25;
	default:	goto yy13;
	}
yy21:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy26;
	default:	goto yy13;
	}
yy22:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy27;
	default:	goto yy13;
	}
yy23:
	yych = *++cur;
	switch (yych) {
	case 'V':
	case 'v':	goto yy28;
	default:	goto yy13;
	}
yy24:
	yych = *++cur;
	switch (yych) {
	case 'D':	goto yy29;
	default:	goto yy13;
	}
yy25:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy30;
	default:	goto yy13;
	}
yy26:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy31;
	default:	goto yy13;
	}
yy27:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy32;
	default:	goto yy13;
	}
yy28:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy33;
	default:	goto yy13;
	}
yy29:
	yych = *++cur;
	switch (yych) {
	case ' ':	goto yy34;
	default:	goto yy13;
	}
yy30:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy35;
	default:	goto yy13;
	}
yy31:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy36;
	default:	goto yy13;
	}
yy32:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy37;
	default:	goto yy13;
	}
yy33:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy38;
	default:	goto yy13;
	}
yy34:
	yych = *++cur;
	switch (yych) {
	case 'C':	goto yy39;
	default:	goto yy13;
	}
yy35:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy40;
	default:	goto yy13;
	}
yy36:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy41;
	default:	goto yy13;
	}
yy37:
	yych = *++cur;
	switch (yych) {
	case ':':	goto yy42;
	default:	goto yy13;
	}
yy38:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy44;
	default:	goto yy13;
	}
yy39:
	yych = *++cur;
	switch (yych) {
	case 'O':	goto yy45;
	default:	goto yy13;
	}
yy40:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy46;
	default:	goto yy13;
	}
yy41:
	yych = *++cur;
	switch (yych) {
	case ':':	goto yy47;
	default:	goto yy13;
	}
yy42:
	yyaccept = 1;
	yych = *(marker = ++cur);
	switch (yych) {
	case ' ':	goto yy49;
	default:	goto yy13;
	}
yy43:
#line 433 "parse_pack.re"
	{ tok = cur; goto yyc_command; }
#line 356 "parse_pack.c"
yy44:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy50;
	default:	goto yy13;
	}
yy45:
	yych = *++cur;
	switch (yych) {
	case 'M':	goto yy51;
	default:	goto yy13;
	}
yy46:
	yych = *++cur;
	switch (yych) {
	case ':':	goto yy52;
	default:	goto yy13;
	}
yy47:
	++cur;
	yych = *cur;
	switch (yych) {
	case '\n':	goto yy5;
	case '\r':	goto yy54;
	default:	goto yy47;
	}
yy49:
	yyaccept = 1;
	yych = *(marker = ++cur);
	goto yy13;
yy50:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy56;
	default:	goto yy13;
	}
yy51:
	yych = *++cur;
	switch (yych) {
	case 'M':	goto yy57;
	default:	goto yy13;
	}
yy52:
	++cur;
	yych = *cur;
	switch (yych) {
	case '\n':	goto yy5;
	case '\r':	goto yy58;
	default:	goto yy52;
	}
yy54:
	++cur;
	yych = *cur;
	switch (yych) {
	case '\n':	goto yy60;
	case '\r':	goto yy54;
	default:	goto yy47;
	}
yy56:
	yych = *++cur;
	switch (yych) {
	case ':':	goto yy62;
	default:	goto yy13;
	}
yy57:
	yych = *++cur;
	switch (yych) {
	case 'A':	goto yy64;
	default:	goto yy13;
	}
yy58:
	++cur;
	yych = *cur;
	switch (yych) {
	case '\n':	goto yy65;
	case '\r':	goto yy58;
	default:	goto yy52;
	}
yy60:
	++cur;
#line 432 "parse_pack.re"
	{ CMD_HEADER(8, Message); }
#line 441 "parse_pack.c"
yy62:
	++cur;
	yych = *cur;
	switch (yych) {
	case '\n':	goto yy5;
	case '\r':	goto yy67;
	default:	goto yy62;
	}
yy64:
	yych = *++cur;
	switch (yych) {
	case 'N':	goto yy69;
	default:	goto yy13;
	}
yy65:
	++cur;
#line 431 "parse_pack.re"
	{ CMD_HEADER(9, ActionID); }
#line 460 "parse_pack.c"
yy67:
	++cur;
	yych = *cur;
	switch (yych) {
	case '\n':	goto yy70;
	case '\r':	goto yy67;
	default:	goto yy62;
	}
yy69:
	yych = *++cur;
	switch (yych) {
	case 'D':	goto yy72;
	default:	goto yy13;
	}
yy70:
	++cur;
#line 430 "parse_pack.re"
	{ CMD_HEADER(10, Privilege); }
#line 479 "parse_pack.c"
yy72:
	yych = *++cur;
	switch (yych) {
	case '-':	goto yy73;
	default:	goto yy13;
	}
yy73:
	yych = *++cur;
	switch (yych) {
	case '-':	goto yy74;
	default:	goto yy13;
	}
yy74:
	yych = *++cur;
	switch (yych) {
	case '\r':	goto yy75;
	default:	goto yy13;
	}
yy75:
	yych = *++cur;
	switch (yych) {
	case '\n':	goto yy76;
	default:	goto yy12;
	}
yy76:
	yyaccept = 2;
	yych = *(marker = ++cur);
	switch (yych) {
	case '\r':	goto yy77;
	default:	goto yy6;
	}
yy77:
	yych = *++cur;
	switch (yych) {
	case '\n':	goto yy79;
	default:	goto yy78;
	}
yy78:
	cur = marker;
	switch (yyaccept) {
	case 0: 	goto yy4;
	case 1: 	goto yy43;
	default:	goto yy6;
	}
yy79:
	++cur;
#line 435 "parse_pack.re"
	{
              len = cur - tok - 19; // output minus command end tag
              char *val = substr(tok, len, 0);
              amipack_append (pack, Output, val);
              free (val);
              goto done;
            }
#line 534 "parse_pack.c"
/* *********************************** */
yyc_key:
	yych = *cur;
	switch (yych) {
	case '\r':	goto yy86;
	case ' ':	goto yy87;
	case ':':	goto yy89;
	case 'A':
	case 'a':	goto yy92;
	case 'B':
	case 'b':	goto yy93;
	case 'C':
	case 'c':	goto yy94;
	case 'D':
	case 'd':	goto yy95;
	case 'E':
	case 'e':	goto yy96;
	case 'F':
	case 'f':	goto yy97;
	case 'H':
	case 'h':	goto yy98;
	case 'I':
	case 'i':	goto yy99;
	case 'K':
	case 'k':	goto yy100;
	case 'L':
	case 'l':	goto yy101;
	case 'M':
	case 'm':	goto yy102;
	case 'N':
	case 'n':	goto yy103;
	case 'O':
	case 'o':	goto yy104;
	case 'P':
	case 'p':	goto yy105;
	case 'Q':
	case 'q':	goto yy106;
	case 'R':
	case 'r':	goto yy107;
	case 'S':
	case 's':	goto yy108;
	case 'T':
	case 't':	goto yy109;
	case 'U':
	case 'u':	goto yy110;
	case 'V':
	case 'v':	goto yy111;
	case 'W':
	case 'w':	goto yy112;
	default:	goto yy83;
	}
yy83:
	++cur;
	yych = *cur;
yy84:
	switch (yych) {
	case ' ':
	case ':':	goto yy85;
	default:	goto yy83;
	}
yy85:
#line 409 "parse_pack.re"
	{
              len = cur - tok;
              hdr_type = HDR_UNKNOWN;
              hdr_name = substr (tok, len, 0);
              goto yyc_key;
            }
#line 603 "parse_pack.c"
yy86:
	yych = *++cur;
	switch (yych) {
	case '\n':	goto yy113;
	default:	goto yy84;
	}
yy87:
	++cur;
#line 223 "parse_pack.re"
	{
              if (hdr_name) free (hdr_name);
              amipack_destroy (pack);
              return NULL;
            }
#line 618 "parse_pack.c"
yy89:
	yyaccept = 0;
	marker = ++cur;
	yych = *cur;
	switch (yych) {
	case '\r':	goto yy114;
	case ' ':	goto yy89;
	default:	goto yy91;
	}
yy91:
#line 230 "parse_pack.re"
	{ tok = cur; goto yyc_value; }
#line 631 "parse_pack.c"
yy92:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy116;
	case 'D':
	case 'd':	goto yy117;
	case 'G':
	case 'g':	goto yy118;
	case 'M':
	case 'm':	goto yy119;
	case 'N':
	case 'n':	goto yy120;
	case 'P':
	case 'p':	goto yy121;
	case 'S':
	case 's':	goto yy122;
	case 'U':
	case 'u':	goto yy123;
	default:	goto yy84;
	}
yy93:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy124;
	case 'R':
	case 'r':	goto yy125;
	default:	goto yy84;
	}
yy94:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy126;
	case 'H':
	case 'h':	goto yy127;
	case 'I':
	case 'i':	goto yy128;
	case 'O':
	case 'o':	goto yy129;
	default:	goto yy84;
	}
yy95:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy130;
	case 'E':
	case 'e':	goto yy131;
	case 'I':
	case 'i':	goto yy132;
	case 'O':
	case 'o':	goto yy133;
	case 'U':
	case 'u':	goto yy134;
	case 'Y':
	case 'y':	goto yy135;
	default:	goto yy84;
	}
yy96:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy136;
	case 'V':
	case 'v':	goto yy137;
	case 'X':
	case 'x':	goto yy138;
	default:	goto yy84;
	}
yy97:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy139;
	case 'I':
	case 'i':	goto yy140;
	case 'O':
	case 'o':	goto yy141;
	case 'R':
	case 'r':	goto yy142;
	default:	goto yy84;
	}
yy98:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy143;
	default:	goto yy84;
	}
yy99:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy144;
	default:	goto yy84;
	}
yy100:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy145;
	default:	goto yy84;
	}
yy101:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy146;
	case 'I':
	case 'i':	goto yy147;
	case 'O':
	case 'o':	goto yy148;
	default:	goto yy84;
	}
yy102:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy149;
	case 'D':
	case 'd':	goto yy150;
	case 'E':
	case 'e':	goto yy151;
	case 'I':
	case 'i':	goto yy152;
	case 'O':
	case 'o':	goto yy153;
	default:	goto yy84;
	}
yy103:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy154;
	default:	goto yy84;
	}
yy104:
	yych = *++cur;
	switch (yych) {
	case 'B':
	case 'b':	goto yy155;
	case 'L':
	case 'l':	goto yy156;
	case 'U':
	case 'u':	goto yy157;
	default:	goto yy84;
	}
yy105:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy158;
	case 'E':
	case 'e':	goto yy159;
	case 'I':
	case 'i':	goto yy160;
	case 'O':
	case 'o':	goto yy161;
	case 'R':
	case 'r':	goto yy162;
	default:	goto yy84;
	}
yy106:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy163;
	default:	goto yy84;
	}
yy107:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy164;
	default:	goto yy84;
	}
yy108:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy165;
	case 'H':
	case 'h':	goto yy166;
	case 'I':
	case 'i':	goto yy167;
	case 'O':
	case 'o':	goto yy168;
	case 'R':
	case 'r':	goto yy169;
	case 'T':
	case 't':	goto yy170;
	case 'U':
	case 'u':	goto yy171;
	default:	goto yy84;
	}
yy109:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy172;
	case 'R':
	case 'r':	goto yy173;
	default:	goto yy84;
	}
yy110:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy174;
	case 'S':
	case 's':	goto yy175;
	default:	goto yy84;
	}
yy111:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy176;
	case 'O':
	case 'o':	goto yy177;
	default:	goto yy84;
	}
yy112:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy178;
	default:	goto yy84;
	}
yy113:
	yych = *++cur;
	switch (yych) {
	case '\r':	goto yy179;
	default:	goto yy84;
	}
yy114:
	yych = *++cur;
	switch (yych) {
	case '\n':	goto yy180;
	default:	goto yy115;
	}
yy115:
	cur = marker;
	if (yyaccept == 0) {
		goto yy91;
	} else {
		goto yy768;
	}
yy116:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy181;
	case 'L':
	case 'l':	goto yy182;
	case 'T':
	case 't':	goto yy184;
	default:	goto yy84;
	}
yy117:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy185;
	default:	goto yy84;
	}
yy118:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy186;
	default:	goto yy84;
	}
yy119:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy187;
	default:	goto yy84;
	}
yy120:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy188;
	default:	goto yy84;
	}
yy121:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy189;
	default:	goto yy84;
	}
yy122:
	yych = *++cur;
	switch (yych) {
	case 'Y':
	case 'y':	goto yy190;
	default:	goto yy84;
	}
yy123:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy191;
	default:	goto yy84;
	}
yy124:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy192;
	default:	goto yy84;
	}
yy125:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy193;
	default:	goto yy84;
	}
yy126:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy194;
	case 'U':
	case 'u':	goto yy195;
	default:	goto yy84;
	}
yy127:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy196;
	default:	goto yy84;
	}
yy128:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy197;
	default:	goto yy84;
	}
yy129:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy198;
	case 'M':
	case 'm':	goto yy199;
	case 'N':
	case 'n':	goto yy200;
	case 'U':
	case 'u':	goto yy201;
	default:	goto yy84;
	}
yy130:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy202;
	default:	goto yy84;
	}
yy131:
	yych = *++cur;
	switch (yych) {
	case 'F':
	case 'f':	goto yy203;
	case 'S':
	case 's':	goto yy204;
	default:	goto yy84;
	}
yy132:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy205;
	case 'R':
	case 'r':	goto yy206;
	case 'S':
	case 's':	goto yy207;
	default:	goto yy84;
	}
yy133:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy208;
	default:	goto yy84;
	}
yy134:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy209;
	default:	goto yy84;
	}
yy135:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy210;
	default:	goto yy84;
	}
yy136:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy211;
	default:	goto yy84;
	}
yy137:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy212;
	default:	goto yy84;
	}
yy138:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy213;
	default:	goto yy84;
	}
yy139:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy214;
	default:	goto yy84;
	}
yy140:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy215;
	default:	goto yy84;
	}
yy141:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy216;
	default:	goto yy84;
	}
yy142:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy217;
	default:	goto yy84;
	}
yy143:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy218;
	default:	goto yy84;
	}
yy144:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy219;
	default:	goto yy84;
	}
yy145:
	yych = *++cur;
	switch (yych) {
	case 'Y':
	case 'y':	goto yy220;
	default:	goto yy84;
	}
yy146:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy222;
	default:	goto yy84;
	}
yy147:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy223;
	case 'S':
	case 's':	goto yy224;
	default:	goto yy84;
	}
yy148:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy225;
	case 'G':
	case 'g':	goto yy226;
	default:	goto yy84;
	}
yy149:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy227;
	default:	goto yy84;
	}
yy150:
	yych = *++cur;
	switch (yych) {
	case '5':	goto yy228;
	default:	goto yy84;
	}
yy151:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy229;
	case 'S':
	case 's':	goto yy230;
	default:	goto yy84;
	}
yy152:
	yych = *++cur;
	switch (yych) {
	case 'X':
	case 'x':	goto yy231;
	default:	goto yy84;
	}
yy153:
	yych = *++cur;
	switch (yych) {
	case 'H':
	case 'h':	goto yy233;
	default:	goto yy84;
	}
yy154:
	yych = *++cur;
	switch (yych) {
	case 'W':
	case 'w':	goto yy234;
	default:	goto yy84;
	}
yy155:
	yych = *++cur;
	switch (yych) {
	case 'J':
	case 'j':	goto yy235;
	default:	goto yy84;
	}
yy156:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy236;
	default:	goto yy84;
	}
yy157:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy237;
	default:	goto yy84;
	}
yy158:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy238;
	case 'U':
	case 'u':	goto yy239;
	default:	goto yy84;
	}
yy159:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy240;
	case 'N':
	case 'n':	goto yy241;
	default:	goto yy84;
	}
yy160:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy242;
	default:	goto yy84;
	}
yy161:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy243;
	default:	goto yy84;
	}
yy162:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy244;
	default:	goto yy84;
	}
yy163:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy245;
	default:	goto yy84;
	}
yy164:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy246;
	case 'G':
	case 'g':	goto yy247;
	case 'M':
	case 'm':	goto yy248;
	case 'S':
	case 's':	goto yy249;
	default:	goto yy84;
	}
yy165:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy250;
	default:	goto yy84;
	}
yy166:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy251;
	default:	goto yy84;
	}
yy167:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy252;
	default:	goto yy84;
	}
yy168:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy253;
	default:	goto yy84;
	}
yy169:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy254;
	default:	goto yy84;
	}
yy170:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy255;
	default:	goto yy84;
	}
yy171:
	yych = *++cur;
	switch (yych) {
	case 'B':
	case 'b':	goto yy256;
	default:	goto yy84;
	}
yy172:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy257;
	default:	goto yy84;
	}
yy173:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy258;
	default:	goto yy84;
	}
yy174:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy259;
	default:	goto yy84;
	}
yy175:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy260;
	default:	goto yy84;
	}
yy176:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy261;
	case 'R':
	case 'r':	goto yy263;
	default:	goto yy84;
	}
yy177:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy264;
	default:	goto yy84;
	}
yy178:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy265;
	default:	goto yy84;
	}
yy179:
	yych = *++cur;
	switch (yych) {
	case '\n':	goto yy266;
	default:	goto yy84;
	}
yy180:
	yych = *++cur;
	ctxmarker = cur;
	switch (yych) {
	case '\r':	goto yy268;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':	goto yy269;
	default:	goto yy115;
	}
yy181:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy271;
	default:	goto yy84;
	}
yy182:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy183;
	default:	goto yy83;
	}
yy183:
#line 272 "parse_pack.re"
	{ SET_HEADER(ACL); }
#line 1440 "parse_pack.c"
yy184:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy272;
	default:	goto yy84;
	}
yy185:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy273;
	default:	goto yy84;
	}
yy186:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy274;
	default:	goto yy84;
	}
yy187:
	yych = *++cur;
	switch (yych) {
	case 'F':
	case 'f':	goto yy275;
	default:	goto yy84;
	}
yy188:
	yych = *++cur;
	switch (yych) {
	case 'W':
	case 'w':	goto yy276;
	default:	goto yy84;
	}
yy189:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy277;
	case 'L':
	case 'l':	goto yy278;
	default:	goto yy84;
	}
yy190:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy279;
	default:	goto yy84;
	}
yy191:
	yych = *++cur;
	switch (yych) {
	case 'H':
	case 'h':	goto yy280;
	default:	goto yy84;
	}
yy192:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy281;
	default:	goto yy84;
	}
yy193:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy282;
	default:	goto yy84;
	}
yy194:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy283;
	default:	goto yy84;
	}
yy195:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy284;
	default:	goto yy84;
	}
yy196:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy285;
	default:	goto yy84;
	}
yy197:
	yych = *++cur;
	switch (yych) {
	case '-':	goto yy286;
	default:	goto yy84;
	}
yy198:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy287;
	default:	goto yy84;
	}
yy199:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy288;
	default:	goto yy84;
	}
yy200:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy289;
	case 'T':
	case 't':	goto yy290;
	default:	goto yy84;
	}
yy201:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy291;
	default:	goto yy84;
	}
yy202:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy292;
	default:	goto yy84;
	}
yy203:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy294;
	default:	goto yy84;
	}
yy204:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy295;
	default:	goto yy84;
	}
yy205:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy296;
	default:	goto yy84;
	}
yy206:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy297;
	default:	goto yy84;
	}
yy207:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy298;
	default:	goto yy84;
	}
yy208:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy299;
	default:	goto yy84;
	}
yy209:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy300;
	default:	goto yy84;
	}
yy210:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy301;
	default:	goto yy84;
	}
yy211:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy302;
	default:	goto yy84;
	}
yy212:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy303;
	default:	goto yy84;
	}
yy213:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy304;
	case 'R':
	case 'r':	goto yy305;
	default:	goto yy84;
	}
yy214:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy306;
	default:	goto yy84;
	}
yy215:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy307;
	default:	goto yy84;
	}
yy216:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy309;
	default:	goto yy84;
	}
yy217:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy310;
	default:	goto yy84;
	}
yy218:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy312;
	default:	goto yy84;
	}
yy219:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy314;
	default:	goto yy84;
	}
yy220:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy221;
	default:	goto yy83;
	}
yy221:
#line 340 "parse_pack.re"
	{ SET_HEADER(Key); }
#line 1708 "parse_pack.c"
yy222:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy315;
	default:	goto yy84;
	}
yy223:
	yych = *++cur;
	switch (yych) {
	case 'K':
	case 'k':	goto yy316;
	default:	goto yy84;
	}
yy224:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy318;
	default:	goto yy84;
	}
yy225:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy319;
	default:	goto yy84;
	}
yy226:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy320;
	default:	goto yy84;
	}
yy227:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy321;
	default:	goto yy84;
	}
yy228:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy322;
	default:	goto yy84;
	}
yy229:
	yych = *++cur;
	switch (yych) {
	case 'B':
	case 'b':	goto yy323;
	default:	goto yy84;
	}
yy230:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy324;
	default:	goto yy84;
	}
yy231:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy232;
	default:	goto yy83;
	}
yy232:
#line 354 "parse_pack.re"
	{ SET_HEADER(Mix); }
#line 1782 "parse_pack.c"
yy233:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy325;
	default:	goto yy84;
	}
yy234:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy326;
	case 'N':
	case 'n':	goto yy327;
	default:	goto yy84;
	}
yy235:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy328;
	default:	goto yy84;
	}
yy236:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy329;
	case 'M':
	case 'm':	goto yy330;
	case 'N':
	case 'n':	goto yy331;
	default:	goto yy84;
	}
yy237:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy332;
	case 'P':
	case 'p':	goto yy333;
	default:	goto yy84;
	}
yy238:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy334;
	default:	goto yy84;
	}
yy239:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy335;
	default:	goto yy84;
	}
yy240:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy336;
	default:	goto yy84;
	}
yy241:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy338;
	default:	goto yy84;
	}
yy242:
	yych = *++cur;
	switch (yych) {
	case 'K':
	case 'k':	goto yy339;
	default:	goto yy84;
	}
yy243:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy340;
	default:	goto yy84;
	}
yy244:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy341;
	case 'V':
	case 'v':	goto yy342;
	default:	goto yy84;
	}
yy245:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy343;
	default:	goto yy84;
	}
yy246:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy344;
	default:	goto yy84;
	}
yy247:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy345;
	default:	goto yy84;
	}
yy248:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy346;
	default:	goto yy84;
	}
yy249:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy347;
	case 'P':
	case 'p':	goto yy348;
	case 'T':
	case 't':	goto yy349;
	default:	goto yy84;
	}
yy250:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy350;
	case 'R':
	case 'r':	goto yy351;
	default:	goto yy84;
	}
yy251:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy352;
	default:	goto yy84;
	}
yy252:
	yych = *++cur;
	switch (yych) {
	case '-':	goto yy353;
	case 'L':
	case 'l':	goto yy354;
	default:	goto yy84;
	}
yy253:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy355;
	default:	goto yy84;
	}
yy254:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy356;
	default:	goto yy84;
	}
yy255:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy357;
	case 'T':
	case 't':	goto yy358;
	default:	goto yy84;
	}
yy256:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy359;
	default:	goto yy84;
	}
yy257:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy360;
	default:	goto yy84;
	}
yy258:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy362;
	default:	goto yy84;
	}
yy259:
	yych = *++cur;
	switch (yych) {
	case 'Q':
	case 'q':	goto yy363;
	default:	goto yy84;
	}
yy260:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy364;
	default:	goto yy84;
	}
yy261:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy262;
	case 'U':
	case 'u':	goto yy366;
	default:	goto yy83;
	}
yy262:
#line 404 "parse_pack.re"
	{ SET_HEADER(Val); }
#line 2010 "parse_pack.c"
yy263:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy367;
	default:	goto yy84;
	}
yy264:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy368;
	default:	goto yy84;
	}
yy265:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy369;
	default:	goto yy84;
	}
yy266:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy267;
	default:	goto yy83;
	}
yy267:
#line 228 "parse_pack.re"
	{ goto done; }
#line 2042 "parse_pack.c"
yy268:
	yych = *++cur;
	switch (yych) {
	case '\n':	goto yy370;
	default:	goto yy115;
	}
yy269:
	++cur;
	cur = ctxmarker;
#line 231 "parse_pack.re"
	{
              tok = cur;
              if (hdr_type == HDR_UNKNOWN) {
                amipack_append_unknown (pack, hdr_name, NULL);
              } else {
                amipack_append (pack, hdr_type, NULL);
              }
              goto yyc_key;
            }
#line 2062 "parse_pack.c"
yy271:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy372;
	default:	goto yy84;
	}
yy272:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy373;
	default:	goto yy84;
	}
yy273:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy374;
	default:	goto yy84;
	}
yy274:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy375;
	default:	goto yy84;
	}
yy275:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy377;
	default:	goto yy84;
	}
yy276:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy378;
	default:	goto yy84;
	}
yy277:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy379;
	default:	goto yy84;
	}
yy278:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy380;
	default:	goto yy84;
	}
yy279:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy381;
	default:	goto yy84;
	}
yy280:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy383;
	default:	goto yy84;
	}
yy281:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy384;
	default:	goto yy84;
	}
yy282:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy385;
	default:	goto yy84;
	}
yy283:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy386;
	case 'G':
	case 'g':	goto yy387;
	case 'S':
	case 's':	goto yy388;
	default:	goto yy84;
	}
yy284:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy389;
	default:	goto yy84;
	}
yy285:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy391;
	case 'O':
	case 'o':	goto yy392;
	default:	goto yy84;
	}
yy286:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy393;
	default:	goto yy84;
	}
yy287:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy394;
	default:	goto yy84;
	}
yy288:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy395;
	default:	goto yy84;
	}
yy289:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy396;
	default:	goto yy84;
	}
yy290:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy397;
	default:	goto yy84;
	}
yy291:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy398;
	default:	goto yy84;
	}
yy292:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy293;
	default:	goto yy83;
	}
yy293:
#line 311 "parse_pack.re"
	{ SET_HEADER(Data); }
#line 2226 "parse_pack.c"
yy294:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy400;
	default:	goto yy84;
	}
yy295:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy401;
	case 'U':
	case 'u':	goto yy402;
	default:	goto yy84;
	}
yy296:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy403;
	default:	goto yy84;
	}
yy297:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy404;
	default:	goto yy84;
	}
yy298:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy405;
	default:	goto yy84;
	}
yy299:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy406;
	default:	goto yy84;
	}
yy300:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy407;
	default:	goto yy84;
	}
yy301:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy408;
	default:	goto yy84;
	}
yy302:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy409;
	default:	goto yy84;
	}
yy303:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy410;
	default:	goto yy84;
	}
yy304:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy412;
	default:	goto yy84;
	}
yy305:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy414;
	default:	goto yy84;
	}
yy306:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy415;
	default:	goto yy84;
	}
yy307:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy308;
	case 'N':
	case 'n':	goto yy416;
	default:	goto yy83;
	}
yy308:
#line 334 "parse_pack.re"
	{ SET_HEADER(File); }
#line 2332 "parse_pack.c"
yy309:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy417;
	default:	goto yy84;
	}
yy310:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy311;
	default:	goto yy83;
	}
yy311:
#line 337 "parse_pack.re"
	{ SET_HEADER(From); }
#line 2350 "parse_pack.c"
yy312:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy313;
	default:	goto yy83;
	}
yy313:
#line 338 "parse_pack.re"
	{ SET_HEADER(Hint); }
#line 2361 "parse_pack.c"
yy314:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy418;
	default:	goto yy84;
	}
yy315:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy419;
	case 'C':
	case 'c':	goto yy420;
	case 'D':
	case 'd':	goto yy421;
	default:	goto yy84;
	}
yy316:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy317;
	default:	goto yy83;
	}
yy317:
#line 344 "parse_pack.re"
	{ SET_HEADER(Link); }
#line 2390 "parse_pack.c"
yy318:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy422;
	default:	goto yy84;
	}
yy319:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy423;
	case 'T':
	case 't':	goto yy424;
	default:	goto yy84;
	}
yy320:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy425;
	default:	goto yy84;
	}
yy321:
	yych = *++cur;
	switch (yych) {
	case 'B':
	case 'b':	goto yy426;
	default:	goto yy84;
	}
yy322:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy427;
	default:	goto yy84;
	}
yy323:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy428;
	default:	goto yy84;
	}
yy324:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy429;
	default:	goto yy84;
	}
yy325:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy430;
	default:	goto yy84;
	}
yy326:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy431;
	default:	goto yy84;
	}
yy327:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy432;
	default:	goto yy84;
	}
yy328:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy433;
	default:	goto yy84;
	}
yy329:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy434;
	default:	goto yy84;
	}
yy330:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy435;
	default:	goto yy84;
	}
yy331:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy436;
	default:	goto yy84;
	}
yy332:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy437;
	default:	goto yy84;
	}
yy333:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy438;
	default:	goto yy84;
	}
yy334:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy439;
	default:	goto yy84;
	}
yy335:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy440;
	default:	goto yy84;
	}
yy336:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy337;
	case 'S':
	case 's':	goto yy441;
	default:	goto yy83;
	}
yy337:
#line 366 "parse_pack.re"
	{ SET_HEADER(Peer); }
#line 2531 "parse_pack.c"
yy338:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy442;
	default:	goto yy84;
	}
yy339:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy443;
	default:	goto yy84;
	}
yy340:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy444;
	default:	goto yy84;
	}
yy341:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy445;
	default:	goto yy84;
	}
yy342:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy446;
	default:	goto yy84;
	}
yy343:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy447;
	default:	goto yy84;
	}
yy344:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy449;
	default:	goto yy84;
	}
yy345:
	yych = *++cur;
	switch (yych) {
	case 'X':
	case 'x':	goto yy450;
	default:	goto yy84;
	}
yy346:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy451;
	default:	goto yy84;
	}
yy347:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy452;
	default:	goto yy84;
	}
yy348:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy453;
	default:	goto yy84;
	}
yy349:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy454;
	default:	goto yy84;
	}
yy350:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy455;
	default:	goto yy84;
	}
yy351:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy456;
	default:	goto yy84;
	}
yy352:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy457;
	default:	goto yy84;
	}
yy353:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy458;
	case 'F':
	case 'f':	goto yy459;
	case 'N':
	case 'n':	goto yy460;
	default:	goto yy84;
	}
yy354:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy461;
	default:	goto yy84;
	}
yy355:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy462;
	default:	goto yy84;
	}
yy356:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy463;
	default:	goto yy84;
	}
yy357:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy464;
	default:	goto yy84;
	}
yy358:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy465;
	case 'U':
	case 'u':	goto yy467;
	default:	goto yy84;
	}
yy359:
	yych = *++cur;
	switch (yych) {
	case 'V':
	case 'v':	goto yy468;
	default:	goto yy84;
	}
yy360:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy361;
	case 'O':
	case 'o':	goto yy469;
	default:	goto yy83;
	}
yy361:
#line 395 "parse_pack.re"
	{ SET_HEADER(Time); }
#line 2704 "parse_pack.c"
yy362:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy470;
	default:	goto yy84;
	}
yy363:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy471;
	default:	goto yy84;
	}
yy364:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy365;
	case 'F':
	case 'f':	goto yy472;
	case 'N':
	case 'n':	goto yy473;
	default:	goto yy83;
	}
yy365:
#line 401 "parse_pack.re"
	{ SET_HEADER(User); }
#line 2733 "parse_pack.c"
yy366:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy474;
	default:	goto yy84;
	}
yy367:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy476;
	default:	goto yy84;
	}
yy368:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy477;
	default:	goto yy84;
	}
yy369:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy478;
	default:	goto yy84;
	}
yy370:
	++cur;
#line 240 "parse_pack.re"
	{
              tok = cur;
              if (hdr_type == HDR_UNKNOWN) {
                amipack_append_unknown (pack, hdr_name, NULL);
                free (hdr_name);
              } else {
                amipack_append (pack, hdr_type, NULL);
              }
              goto done;
            }
#line 2775 "parse_pack.c"
yy372:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy479;
	default:	goto yy84;
	}
yy373:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy480;
	default:	goto yy84;
	}
yy374:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy482;
	default:	goto yy84;
	}
yy375:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy376;
	default:	goto yy83;
	}
yy376:
#line 277 "parse_pack.re"
	{ SET_HEADER(Agent); }
#line 2807 "parse_pack.c"
yy377:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy483;
	default:	goto yy84;
	}
yy378:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy484;
	default:	goto yy84;
	}
yy379:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy485;
	default:	goto yy84;
	}
yy380:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy487;
	default:	goto yy84;
	}
yy381:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy382;
	default:	goto yy83;
	}
yy382:
#line 282 "parse_pack.re"
	{ SET_HEADER(Async); }
#line 2846 "parse_pack.c"
yy383:
	yych = *++cur;
	switch (yych) {
	case 'Y':
	case 'y':	goto yy488;
	default:	goto yy84;
	}
yy384:
	yych = *++cur;
	switch (yych) {
	case 'B':
	case 'b':	goto yy489;
	default:	goto yy84;
	}
yy385:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy490;
	default:	goto yy84;
	}
yy386:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy491;
	default:	goto yy84;
	}
yy387:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy492;
	default:	goto yy84;
	}
yy388:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy493;
	default:	goto yy84;
	}
yy389:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy390;
	case '-':	goto yy494;
	default:	goto yy83;
	}
yy390:
#line 294 "parse_pack.re"
	{ SET_HEADER(Cause); }
#line 2900 "parse_pack.c"
yy391:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy495;
	default:	goto yy84;
	}
yy392:
	yych = *++cur;
	switch (yych) {
	case 'B':
	case 'b':	goto yy496;
	default:	goto yy84;
	}
yy393:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy497;
	default:	goto yy84;
	}
yy394:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy498;
	case 'S':
	case 's':	goto yy499;
	default:	goto yy84;
	}
yy395:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy501;
	default:	goto yy84;
	}
yy396:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy502;
	default:	goto yy84;
	}
yy397:
	yych = *++cur;
	switch (yych) {
	case 'X':
	case 'x':	goto yy503;
	default:	goto yy84;
	}
yy398:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy399;
	default:	goto yy83;
	}
yy399:
#line 310 "parse_pack.re"
	{ SET_HEADER(Count); }
#line 2962 "parse_pack.c"
yy400:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy504;
	default:	goto yy84;
	}
yy401:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy505;
	default:	goto yy84;
	}
yy402:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy506;
	default:	goto yy84;
	}
yy403:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy507;
	default:	goto yy84;
	}
yy404:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy508;
	default:	goto yy84;
	}
yy405:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy509;
	default:	goto yy84;
	}
yy406:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy510;
	default:	goto yy84;
	}
yy407:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy512;
	default:	goto yy84;
	}
yy408:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy513;
	default:	goto yy84;
	}
yy409:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy514;
	default:	goto yy84;
	}
yy410:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy411;
	case 'L':
	case 'l':	goto yy515;
	case 'S':
	case 's':	goto yy516;
	default:	goto yy83;
	}
yy411:
#line 265 "parse_pack.re"
	{
              amipack_type (pack, AMI_EVENT);
              SET_HEADER(Event);
            }
#line 3050 "parse_pack.c"
yy412:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy413;
	case 'S':
	case 's':	goto yy518;
	default:	goto yy83;
	}
yy413:
#line 328 "parse_pack.re"
	{ SET_HEADER(Exten); }
#line 3063 "parse_pack.c"
yy414:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy519;
	case 'P':
	case 'p':	goto yy520;
	default:	goto yy84;
	}
yy415:
	yych = *++cur;
	switch (yych) {
	case 'Y':
	case 'y':	goto yy521;
	default:	goto yy84;
	}
yy416:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy523;
	default:	goto yy84;
	}
yy417:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy524;
	default:	goto yy84;
	}
yy418:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy526;
	default:	goto yy84;
	}
yy419:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy527;
	default:	goto yy84;
	}
yy420:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy528;
	default:	goto yy84;
	}
yy421:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy529;
	default:	goto yy84;
	}
yy422:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy530;
	default:	goto yy84;
	}
yy423:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy531;
	default:	goto yy84;
	}
yy424:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy532;
	default:	goto yy84;
	}
yy425:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy533;
	case 'T':
	case 't':	goto yy534;
	default:	goto yy84;
	}
yy426:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy535;
	default:	goto yy84;
	}
yy427:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy536;
	default:	goto yy84;
	}
yy428:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy537;
	default:	goto yy84;
	}
yy429:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy538;
	default:	goto yy84;
	}
yy430:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy539;
	default:	goto yy84;
	}
yy431:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy540;
	default:	goto yy84;
	}
yy432:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy541;
	default:	goto yy84;
	}
yy433:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy542;
	default:	goto yy84;
	}
yy434:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy543;
	default:	goto yy84;
	}
yy435:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy544;
	default:	goto yy84;
	}
yy436:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy545;
	default:	goto yy84;
	}
yy437:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy546;
	default:	goto yy84;
	}
yy438:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy547;
	default:	goto yy84;
	}
yy439:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy549;
	default:	goto yy84;
	}
yy440:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy550;
	default:	goto yy84;
	}
yy441:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy552;
	default:	goto yy84;
	}
yy442:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy553;
	default:	goto yy84;
	}
yy443:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy554;
	default:	goto yy84;
	}
yy444:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy555;
	default:	goto yy84;
	}
yy445:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy556;
	default:	goto yy84;
	}
yy446:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy557;
	default:	goto yy84;
	}
yy447:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy448;
	default:	goto yy83;
	}
yy448:
#line 373 "parse_pack.re"
	{ SET_HEADER(Queue); }
#line 3309 "parse_pack.c"
yy449:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy558;
	default:	goto yy84;
	}
yy450:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy560;
	default:	goto yy84;
	}
yy451:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy561;
	default:	goto yy84;
	}
yy452:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy562;
	default:	goto yy84;
	}
yy453:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy563;
	default:	goto yy84;
	}
yy454:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy564;
	default:	goto yy84;
	}
yy455:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy565;
	default:	goto yy84;
	}
yy456:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy566;
	default:	goto yy84;
	}
yy457:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy568;
	default:	goto yy84;
	}
yy458:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy569;
	default:	goto yy84;
	}
yy459:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy570;
	default:	goto yy84;
	}
yy460:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy571;
	default:	goto yy84;
	}
yy461:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy572;
	default:	goto yy84;
	}
yy462:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy573;
	default:	goto yy84;
	}
yy463:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy575;
	default:	goto yy84;
	}
yy464:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy576;
	default:	goto yy84;
	}
yy465:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy466;
	default:	goto yy83;
	}
yy466:
#line 392 "parse_pack.re"
	{ SET_HEADER(State); }
#line 3432 "parse_pack.c"
yy467:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy577;
	default:	goto yy84;
	}
yy468:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy579;
	default:	goto yy84;
	}
yy469:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy580;
	default:	goto yy84;
	}
yy470:
	yych = *++cur;
	switch (yych) {
	case 'F':
	case 'f':	goto yy581;
	default:	goto yy84;
	}
yy471:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy582;
	default:	goto yy84;
	}
yy472:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy583;
	default:	goto yy84;
	}
yy473:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy584;
	default:	goto yy84;
	}
yy474:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy475;
	default:	goto yy83;
	}
yy475:
#line 405 "parse_pack.re"
	{ SET_HEADER(Value); }
#line 3492 "parse_pack.c"
yy476:
	yych = *++cur;
	switch (yych) {
	case 'B':
	case 'b':	goto yy585;
	default:	goto yy84;
	}
yy477:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy586;
	default:	goto yy84;
	}
yy478:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy587;
	default:	goto yy84;
	}
yy479:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy588;
	default:	goto yy84;
	}
yy480:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy481;
	case 'I':
	case 'i':	goto yy590;
	default:	goto yy83;
	}
yy481:
#line 261 "parse_pack.re"
	{
              amipack_type (pack, AMI_ACTION);
              SET_HEADER(Action);
            }
#line 3536 "parse_pack.c"
yy482:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy591;
	default:	goto yy84;
	}
yy483:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy593;
	default:	goto yy84;
	}
yy484:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy594;
	default:	goto yy84;
	}
yy485:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy486;
	default:	goto yy83;
	}
yy486:
#line 280 "parse_pack.re"
	{ SET_HEADER(Append); }
#line 3568 "parse_pack.c"
yy487:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy595;
	default:	goto yy84;
	}
yy488:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy596;
	default:	goto yy84;
	}
yy489:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy597;
	default:	goto yy84;
	}
yy490:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy598;
	case 'T':
	case 't':	goto yy599;
	default:	goto yy84;
	}
yy491:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy600;
	default:	goto yy84;
	}
yy492:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy601;
	default:	goto yy84;
	}
yy493:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy602;
	default:	goto yy84;
	}
yy494:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy603;
	default:	goto yy84;
	}
yy495:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy604;
	default:	goto yy84;
	}
yy496:
	yych = *++cur;
	switch (yych) {
	case 'J':
	case 'j':	goto yy606;
	default:	goto yy84;
	}
yy497:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy607;
	default:	goto yy84;
	}
yy498:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy608;
	default:	goto yy84;
	}
yy499:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy500;
	default:	goto yy83;
	}
yy500:
#line 305 "parse_pack.re"
	{ SET_HEADER(Codecs); }
#line 3665 "parse_pack.c"
yy501:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy609;
	default:	goto yy84;
	}
yy502:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy611;
	default:	goto yy84;
	}
yy503:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy612;
	default:	goto yy84;
	}
yy504:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy614;
	default:	goto yy84;
	}
yy505:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy615;
	default:	goto yy84;
	}
yy506:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy616;
	default:	goto yy84;
	}
yy507:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy617;
	case 'R':
	case 'r':	goto yy618;
	default:	goto yy84;
	}
yy508:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy619;
	default:	goto yy84;
	}
yy509:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy620;
	default:	goto yy84;
	}
yy510:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy511;
	default:	goto yy83;
	}
yy511:
#line 322 "parse_pack.re"
	{ SET_HEADER(Domain); }
#line 3741 "parse_pack.c"
yy512:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy621;
	default:	goto yy84;
	}
yy513:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy622;
	default:	goto yy84;
	}
yy514:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy624;
	default:	goto yy84;
	}
yy515:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy626;
	default:	goto yy84;
	}
yy516:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy517;
	default:	goto yy83;
	}
yy517:
#line 327 "parse_pack.re"
	{ SET_HEADER(EventsHdr); }
#line 3780 "parse_pack.c"
yy518:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy627;
	default:	goto yy84;
	}
yy519:
	yych = *++cur;
	switch (yych) {
	case 'H':
	case 'h':	goto yy628;
	case 'O':
	case 'o':	goto yy629;
	default:	goto yy84;
	}
yy520:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy630;
	default:	goto yy84;
	}
yy521:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy522;
	default:	goto yy83;
	}
yy522:
#line 333 "parse_pack.re"
	{ SET_HEADER(Family); }
#line 3814 "parse_pack.c"
yy523:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy631;
	default:	goto yy84;
	}
yy524:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy525;
	default:	goto yy83;
	}
yy525:
#line 336 "parse_pack.re"
	{ SET_HEADER(Format); }
#line 3832 "parse_pack.c"
yy526:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy632;
	default:	goto yy84;
	}
yy527:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy633;
	default:	goto yy84;
	}
yy528:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy634;
	default:	goto yy84;
	}
yy529:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy635;
	default:	goto yy84;
	}
yy530:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy636;
	default:	goto yy84;
	}
yy531:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy637;
	default:	goto yy84;
	}
yy532:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy638;
	default:	goto yy84;
	}
yy533:
	yych = *++cur;
	switch (yych) {
	case 'H':
	case 'h':	goto yy639;
	default:	goto yy84;
	}
yy534:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy640;
	default:	goto yy84;
	}
yy535:
	yych = *++cur;
	switch (yych) {
	case 'X':
	case 'x':	goto yy641;
	default:	goto yy84;
	}
yy536:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy643;
	default:	goto yy84;
	}
yy537:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy644;
	default:	goto yy84;
	}
yy538:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy645;
	default:	goto yy84;
	}
yy539:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy647;
	default:	goto yy84;
	}
yy540:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy648;
	default:	goto yy84;
	}
yy541:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy649;
	default:	goto yy84;
	}
yy542:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy651;
	default:	goto yy84;
	}
yy543:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy652;
	default:	goto yy84;
	}
yy544:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy653;
	default:	goto yy84;
	}
yy545:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy654;
	default:	goto yy84;
	}
yy546:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy656;
	default:	goto yy84;
	}
yy547:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy548;
	default:	goto yy83;
	}
yy548:
#line 363 "parse_pack.re"
	{ SET_HEADER(Output); }
#line 3990 "parse_pack.c"
yy549:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy657;
	default:	goto yy84;
	}
yy550:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy551;
	default:	goto yy83;
	}
yy551:
#line 365 "parse_pack.re"
	{ SET_HEADER(Paused); }
#line 4008 "parse_pack.c"
yy552:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy658;
	default:	goto yy84;
	}
yy553:
	yych = *++cur;
	switch (yych) {
	case 'Y':
	case 'y':	goto yy659;
	default:	goto yy84;
	}
yy554:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy661;
	default:	goto yy84;
	}
yy555:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy662;
	default:	goto yy84;
	}
yy556:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy663;
	default:	goto yy84;
	}
yy557:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy664;
	default:	goto yy84;
	}
yy558:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy559;
	default:	goto yy83;
	}
yy559:
#line 374 "parse_pack.re"
	{ SET_HEADER(Reason); }
#line 4061 "parse_pack.c"
yy560:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy665;
	default:	goto yy84;
	}
yy561:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy666;
	default:	goto yy84;
	}
yy562:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy667;
	default:	goto yy84;
	}
yy563:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy668;
	default:	goto yy84;
	}
yy564:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy669;
	default:	goto yy84;
	}
yy565:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy671;
	default:	goto yy84;
	}
yy566:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy567;
	case 'E':
	case 'e':	goto yy673;
	default:	goto yy83;
	}
yy567:
#line 381 "parse_pack.re"
	{ SET_HEADER(Secret); }
#line 4116 "parse_pack.c"
yy568:
	yych = *++cur;
	switch (yych) {
	case 'W':
	case 'w':	goto yy674;
	default:	goto yy84;
	}
yy569:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy675;
	default:	goto yy84;
	}
yy570:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy676;
	default:	goto yy84;
	}
yy571:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy677;
	default:	goto yy84;
	}
yy572:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy678;
	default:	goto yy84;
	}
yy573:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy574;
	default:	goto yy83;
	}
yy574:
#line 389 "parse_pack.re"
	{ SET_HEADER(Source); }
#line 4162 "parse_pack.c"
yy575:
	yych = *++cur;
	switch (yych) {
	case 'Q':
	case 'q':	goto yy679;
	default:	goto yy84;
	}
yy576:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy680;
	default:	goto yy84;
	}
yy577:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy578;
	default:	goto yy83;
	}
yy578:
#line 393 "parse_pack.re"
	{ SET_HEADER(StatusHdr); }
#line 4187 "parse_pack.c"
yy579:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy681;
	default:	goto yy84;
	}
yy580:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy682;
	default:	goto yy84;
	}
yy581:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy684;
	default:	goto yy84;
	}
yy582:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy685;
	default:	goto yy84;
	}
yy583:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy686;
	default:	goto yy84;
	}
yy584:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy687;
	default:	goto yy84;
	}
yy585:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy688;
	default:	goto yy84;
	}
yy586:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy689;
	default:	goto yy84;
	}
yy587:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy690;
	default:	goto yy84;
	}
yy588:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy589;
	case 'C':
	case 'c':	goto yy692;
	default:	goto yy83;
	}
yy589:
#line 270 "parse_pack.re"
	{ SET_HEADER(Account); }
#line 4263 "parse_pack.c"
yy590:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy693;
	default:	goto yy84;
	}
yy591:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy592;
	case '-':	goto yy695;
	default:	goto yy83;
	}
yy592:
#line 274 "parse_pack.re"
	{ SET_HEADER(Address); }
#line 4282 "parse_pack.c"
yy593:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy696;
	default:	goto yy84;
	}
yy594:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy698;
	default:	goto yy84;
	}
yy595:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy699;
	default:	goto yy84;
	}
yy596:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy700;
	default:	goto yy84;
	}
yy597:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy702;
	default:	goto yy84;
	}
yy598:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy703;
	default:	goto yy84;
	}
yy599:
	yych = *++cur;
	switch (yych) {
	case 'Y':
	case 'y':	goto yy704;
	default:	goto yy84;
	}
yy600:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy705;
	default:	goto yy84;
	}
yy601:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy707;
	default:	goto yy84;
	}
yy602:
	yych = *++cur;
	switch (yych) {
	case 'K':
	case 'k':	goto yy708;
	default:	goto yy84;
	}
yy603:
	yych = *++cur;
	switch (yych) {
	case 'X':
	case 'x':	goto yy709;
	default:	goto yy84;
	}
yy604:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy605;
	case '1':	goto yy710;
	case '2':	goto yy712;
	case 'S':
	case 's':	goto yy714;
	case 'T':
	case 't':	goto yy715;
	default:	goto yy83;
	}
yy605:
#line 296 "parse_pack.re"
	{ SET_HEADER(Channel); }
#line 4376 "parse_pack.c"
yy606:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy716;
	default:	goto yy84;
	}
yy607:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy717;
	default:	goto yy84;
	}
yy608:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy718;
	default:	goto yy84;
	}
yy609:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy610;
	default:	goto yy83;
	}
yy610:
#line 306 "parse_pack.re"
	{ SET_HEADER(CommandHdr); }
#line 4408 "parse_pack.c"
yy611:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy719;
	default:	goto yy84;
	}
yy612:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy613;
	default:	goto yy83;
	}
yy613:
#line 309 "parse_pack.re"
	{ SET_HEADER(Context); }
#line 4426 "parse_pack.c"
yy614:
	yych = *++cur;
	switch (yych) {
	case '-':	goto yy720;
	default:	goto yy84;
	}
yy615:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy721;
	default:	goto yy84;
	}
yy616:
	yych = *++cur;
	switch (yych) {
	case 'Q':
	case 'q':	goto yy722;
	default:	goto yy84;
	}
yy617:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy723;
	default:	goto yy84;
	}
yy618:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy724;
	default:	goto yy84;
	}
yy619:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy725;
	default:	goto yy84;
	}
yy620:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy726;
	default:	goto yy84;
	}
yy621:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy727;
	default:	goto yy84;
	}
yy622:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy623;
	default:	goto yy83;
	}
yy623:
#line 324 "parse_pack.re"
	{ SET_HEADER(Dynamic); }
#line 4492 "parse_pack.c"
yy624:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy625;
	default:	goto yy83;
	}
yy625:
#line 325 "parse_pack.re"
	{ SET_HEADER(Endtime); }
#line 4503 "parse_pack.c"
yy626:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy729;
	default:	goto yy84;
	}
yy627:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy730;
	default:	goto yy84;
	}
yy628:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy731;
	default:	goto yy84;
	}
yy629:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy732;
	default:	goto yy84;
	}
yy630:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy733;
	default:	goto yy84;
	}
yy631:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy734;
	default:	goto yy84;
	}
yy632:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy736;
	default:	goto yy84;
	}
yy633:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy737;
	default:	goto yy84;
	}
yy634:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy738;
	default:	goto yy84;
	}
yy635:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy740;
	default:	goto yy84;
	}
yy636:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy742;
	default:	goto yy84;
	}
yy637:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy743;
	default:	goto yy84;
	}
yy638:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy744;
	default:	goto yy84;
	}
yy639:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy746;
	default:	goto yy84;
	}
yy640:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy747;
	default:	goto yy84;
	}
yy641:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy642;
	default:	goto yy83;
	}
yy642:
#line 350 "parse_pack.re"
	{ SET_HEADER(Mailbox); }
#line 4619 "parse_pack.c"
yy643:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy748;
	default:	goto yy84;
	}
yy644:
	yych = *++cur;
	switch (yych) {
	case 'H':
	case 'h':	goto yy749;
	default:	goto yy84;
	}
yy645:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy646;
	default:	goto yy83;
	}
yy646:
#line 353 "parse_pack.re"
	{ SET_HEADER(Message); }
#line 4644 "parse_pack.c"
yy647:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy750;
	default:	goto yy84;
	}
yy648:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy751;
	default:	goto yy84;
	}
yy649:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy650;
	default:	goto yy83;
	}
yy650:
#line 357 "parse_pack.re"
	{ SET_HEADER(Newname); }
#line 4669 "parse_pack.c"
yy651:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy752;
	default:	goto yy84;
	}
yy652:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy753;
	default:	goto yy84;
	}
yy653:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy754;
	default:	goto yy84;
	}
yy654:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy655;
	default:	goto yy83;
	}
yy655:
#line 361 "parse_pack.re"
	{ SET_HEADER(OldName); }
#line 4701 "parse_pack.c"
yy656:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy755;
	default:	goto yy84;
	}
yy657:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy756;
	default:	goto yy84;
	}
yy658:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy757;
	default:	goto yy84;
	}
yy659:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy660;
	default:	goto yy83;
	}
yy660:
#line 368 "parse_pack.re"
	{ SET_HEADER(Penalty); }
#line 4733 "parse_pack.c"
yy661:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy758;
	default:	goto yy84;
	}
yy662:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy759;
	default:	goto yy84;
	}
yy663:
	yych = *++cur;
	switch (yych) {
	case 'Y':
	case 'y':	goto yy761;
	default:	goto yy84;
	}
yy664:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy763;
	default:	goto yy84;
	}
yy665:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy764;
	default:	goto yy84;
	}
yy666:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy765;
	default:	goto yy84;
	}
yy667:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy766;
	default:	goto yy84;
	}
yy668:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy767;
	default:	goto yy84;
	}
yy669:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy670;
	default:	goto yy83;
	}
yy670:
#line 379 "parse_pack.re"
	{ SET_HEADER(Restart); }
#line 4800 "parse_pack.c"
yy671:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy672;
	default:	goto yy83;
	}
yy672:
#line 380 "parse_pack.re"
	{ SET_HEADER(Seconds); }
#line 4811 "parse_pack.c"
yy673:
	yych = *++cur;
	switch (yych) {
	case 'X':
	case 'x':	goto yy769;
	default:	goto yy84;
	}
yy674:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy770;
	default:	goto yy84;
	}
yy675:
	yych = *++cur;
	switch (yych) {
	case 'H':
	case 'h':	goto yy772;
	default:	goto yy84;
	}
yy676:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy773;
	default:	goto yy84;
	}
yy677:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy774;
	default:	goto yy84;
	}
yy678:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy775;
	default:	goto yy84;
	}
yy679:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy776;
	default:	goto yy84;
	}
yy680:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy777;
	default:	goto yy84;
	}
yy681:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy778;
	default:	goto yy84;
	}
yy682:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy683;
	default:	goto yy83;
	}
yy683:
#line 396 "parse_pack.re"
	{ SET_HEADER(Timeout); }
#line 4885 "parse_pack.c"
yy684:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy780;
	default:	goto yy84;
	}
yy685:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy781;
	default:	goto yy84;
	}
yy686:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy783;
	default:	goto yy84;
	}
yy687:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy784;
	default:	goto yy84;
	}
yy688:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy786;
	default:	goto yy84;
	}
yy689:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy788;
	default:	goto yy84;
	}
yy690:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy691;
	default:	goto yy83;
	}
yy691:
#line 408 "parse_pack.re"
	{ SET_HEADER(Waiting); }
#line 4938 "parse_pack.c"
yy692:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy789;
	default:	goto yy84;
	}
yy693:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy694;
	default:	goto yy83;
	}
yy694:
#line 273 "parse_pack.re"
	{ SET_HEADER(ActionID); }
#line 4956 "parse_pack.c"
yy695:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy790;
	case 'P':
	case 'p':	goto yy791;
	default:	goto yy84;
	}
yy696:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy697;
	default:	goto yy83;
	}
yy697:
#line 278 "parse_pack.re"
	{ SET_HEADER(AMAflags); }
#line 4976 "parse_pack.c"
yy698:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy792;
	default:	goto yy84;
	}
yy699:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy793;
	default:	goto yy84;
	}
yy700:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy701;
	default:	goto yy83;
	}
yy701:
#line 283 "parse_pack.re"
	{ SET_HEADER(AuthType); }
#line 5001 "parse_pack.c"
yy702:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy794;
	default:	goto yy84;
	}
yy703:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy795;
	default:	goto yy84;
	}
yy704:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy796;
	default:	goto yy84;
	}
yy705:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy706;
	case '1':	goto yy797;
	case '2':	goto yy799;
	case 'N':
	case 'n':	goto yy801;
	default:	goto yy83;
	}
yy706:
#line 287 "parse_pack.re"
	{ SET_HEADER(CallerID); }
#line 5037 "parse_pack.c"
yy707:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy802;
	default:	goto yy84;
	}
yy708:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy804;
	default:	goto yy84;
	}
yy709:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy805;
	default:	goto yy84;
	}
yy710:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy711;
	default:	goto yy83;
	}
yy711:
#line 297 "parse_pack.re"
	{ SET_HEADER(Channel1); }
#line 5069 "parse_pack.c"
yy712:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy713;
	default:	goto yy83;
	}
yy713:
#line 298 "parse_pack.re"
	{ SET_HEADER(Channel2); }
#line 5080 "parse_pack.c"
yy714:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy807;
	default:	goto yy84;
	}
yy715:
	yych = *++cur;
	switch (yych) {
	case 'Y':
	case 'y':	goto yy808;
	default:	goto yy84;
	}
yy716:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy809;
	default:	goto yy84;
	}
yy717:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy810;
	default:	goto yy84;
	}
yy718:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy811;
	default:	goto yy84;
	}
yy719:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy812;
	default:	goto yy84;
	}
yy720:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy813;
	case 'U':
	case 'u':	goto yy814;
	default:	goto yy84;
	}
yy721:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy815;
	default:	goto yy84;
	}
yy722:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy816;
	default:	goto yy84;
	}
yy723:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy817;
	default:	goto yy84;
	}
yy724:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy818;
	default:	goto yy84;
	}
yy725:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy819;
	default:	goto yy84;
	}
yy726:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy821;
	default:	goto yy84;
	}
yy727:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy728;
	default:	goto yy83;
	}
yy728:
#line 323 "parse_pack.re"
	{ SET_HEADER(Duration); }
#line 5184 "parse_pack.c"
yy729:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy822;
	default:	goto yy84;
	}
yy730:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy824;
	default:	goto yy84;
	}
yy731:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy826;
	default:	goto yy84;
	}
yy732:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy827;
	default:	goto yy84;
	}
yy733:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy828;
	default:	goto yy84;
	}
yy734:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy735;
	default:	goto yy83;
	}
yy735:
#line 335 "parse_pack.re"
	{ SET_HEADER(FileName); }
#line 5230 "parse_pack.c"
yy736:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy829;
	default:	goto yy84;
	}
yy737:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy830;
	default:	goto yy84;
	}
yy738:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy739;
	default:	goto yy83;
	}
yy739:
#line 342 "parse_pack.re"
	{ SET_HEADER(LastCall); }
#line 5255 "parse_pack.c"
yy740:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy741;
	default:	goto yy83;
	}
yy741:
#line 343 "parse_pack.re"
	{ SET_HEADER(LastData); }
#line 5266 "parse_pack.c"
yy742:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy831;
	default:	goto yy84;
	}
yy743:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy833;
	default:	goto yy84;
	}
yy744:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy745;
	default:	goto yy83;
	}
yy745:
#line 347 "parse_pack.re"
	{ SET_HEADER(Location); }
#line 5291 "parse_pack.c"
yy746:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy834;
	default:	goto yy84;
	}
yy747:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy836;
	default:	goto yy84;
	}
yy748:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy838;
	default:	goto yy84;
	}
yy749:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy839;
	default:	goto yy84;
	}
yy750:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy840;
	default:	goto yy84;
	}
yy751:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy841;
	default:	goto yy84;
	}
yy752:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy842;
	default:	goto yy84;
	}
yy753:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy843;
	default:	goto yy84;
	}
yy754:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy844;
	default:	goto yy84;
	}
yy755:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy845;
	default:	goto yy84;
	}
yy756:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy846;
	default:	goto yy84;
	}
yy757:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy847;
	default:	goto yy84;
	}
yy758:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy848;
	default:	goto yy84;
	}
yy759:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy760;
	default:	goto yy83;
	}
yy760:
#line 370 "parse_pack.re"
	{ SET_HEADER(Position); }
#line 5393 "parse_pack.c"
yy761:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy762;
	default:	goto yy83;
	}
yy762:
#line 371 "parse_pack.re"
	{ SET_HEADER(Priority); }
#line 5404 "parse_pack.c"
yy763:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy849;
	default:	goto yy84;
	}
yy764:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy851;
	case 'Y':
	case 'y':	goto yy853;
	default:	goto yy84;
	}
yy765:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy855;
	default:	goto yy84;
	}
yy766:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy856;
	default:	goto yy84;
	}
yy767:
	yyaccept = 1;
	yych = *(marker = ++cur);
	switch (yych) {
	case ' ':	goto yy768;
	case ':':	goto yy857;
	default:	goto yy83;
	}
yy768:
#line 257 "parse_pack.re"
	{
              amipack_type (pack, AMI_RESPONSE);
              SET_HEADER(Response);
            }
#line 5449 "parse_pack.c"
yy769:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy859;
	default:	goto yy84;
	}
yy770:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy771;
	default:	goto yy83;
	}
yy771:
#line 383 "parse_pack.re"
	{ SET_HEADER(ShutdownHdr); }
#line 5467 "parse_pack.c"
yy772:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy860;
	default:	goto yy84;
	}
yy773:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy861;
	case 'U':
	case 'u':	goto yy862;
	default:	goto yy84;
	}
yy774:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy863;
	default:	goto yy84;
	}
yy775:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy864;
	default:	goto yy84;
	}
yy776:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy865;
	default:	goto yy84;
	}
yy777:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy866;
	default:	goto yy84;
	}
yy778:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy779;
	default:	goto yy83;
	}
yy779:
#line 394 "parse_pack.re"
	{ SET_HEADER(SubEvent); }
#line 5522 "parse_pack.c"
yy780:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy868;
	default:	goto yy84;
	}
yy781:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy782;
	case '1':	goto yy869;
	case '2':	goto yy871;
	default:	goto yy83;
	}
yy782:
#line 398 "parse_pack.re"
	{ SET_HEADER(Uniqueid); }
#line 5542 "parse_pack.c"
yy783:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy873;
	default:	goto yy84;
	}
yy784:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy785;
	default:	goto yy83;
	}
yy785:
#line 403 "parse_pack.re"
	{ SET_HEADER(Username); }
#line 5560 "parse_pack.c"
yy786:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy787;
	default:	goto yy83;
	}
yy787:
#line 406 "parse_pack.re"
	{ SET_HEADER(Variable); }
#line 5571 "parse_pack.c"
yy788:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy875;
	default:	goto yy84;
	}
yy789:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy876;
	default:	goto yy84;
	}
yy790:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy877;
	default:	goto yy84;
	}
yy791:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy879;
	default:	goto yy84;
	}
yy792:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy880;
	default:	goto yy84;
	}
yy793:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy882;
	default:	goto yy84;
	}
yy794:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy883;
	default:	goto yy84;
	}
yy795:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy884;
	default:	goto yy84;
	}
yy796:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy885;
	default:	goto yy84;
	}
yy797:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy798;
	default:	goto yy83;
	}
yy798:
#line 288 "parse_pack.re"
	{ SET_HEADER(CallerID1); }
#line 5645 "parse_pack.c"
yy799:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy800;
	default:	goto yy83;
	}
yy800:
#line 289 "parse_pack.re"
	{ SET_HEADER(CallerID2); }
#line 5656 "parse_pack.c"
yy801:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy887;
	case 'U':
	case 'u':	goto yy888;
	default:	goto yy84;
	}
yy802:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy803;
	default:	goto yy83;
	}
yy803:
#line 292 "parse_pack.re"
	{ SET_HEADER(Callgroup); }
#line 5676 "parse_pack.c"
yy804:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy889;
	default:	goto yy84;
	}
yy805:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy806;
	default:	goto yy83;
	}
yy806:
#line 295 "parse_pack.re"
	{ SET_HEADER(Cause_txt); }
#line 5694 "parse_pack.c"
yy807:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy891;
	default:	goto yy84;
	}
yy808:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy892;
	default:	goto yy84;
	}
yy809:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy893;
	default:	goto yy84;
	}
yy810:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy894;
	default:	goto yy84;
	}
yy811:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy895;
	default:	goto yy84;
	}
yy812:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy897;
	default:	goto yy84;
	}
yy813:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy898;
	default:	goto yy84;
	}
yy814:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy899;
	default:	goto yy84;
	}
yy815:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy900;
	default:	goto yy84;
	}
yy816:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy901;
	default:	goto yy84;
	}
yy817:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy902;
	default:	goto yy84;
	}
yy818:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy904;
	default:	goto yy84;
	}
yy819:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy820;
	default:	goto yy83;
	}
yy820:
#line 320 "parse_pack.re"
	{ SET_HEADER(Direction); }
#line 5789 "parse_pack.c"
yy821:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy906;
	default:	goto yy84;
	}
yy822:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy823;
	default:	goto yy83;
	}
yy823:
#line 326 "parse_pack.re"
	{ SET_HEADER(EventList); }
#line 5807 "parse_pack.c"
yy824:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy825;
	default:	goto yy83;
	}
yy825:
#line 329 "parse_pack.re"
	{ SET_HEADER(Extension); }
#line 5818 "parse_pack.c"
yy826:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy907;
	default:	goto yy84;
	}
yy827:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy908;
	default:	goto yy84;
	}
yy828:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy909;
	default:	goto yy84;
	}
yy829:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy910;
	default:	goto yy84;
	}
yy830:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy911;
	default:	goto yy84;
	}
yy831:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy832;
	default:	goto yy83;
	}
yy832:
#line 345 "parse_pack.re"
	{ SET_HEADER(ListItems); }
#line 5864 "parse_pack.c"
yy833:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy912;
	default:	goto yy84;
	}
yy834:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy835;
	default:	goto yy83;
	}
yy835:
#line 348 "parse_pack.re"
	{ SET_HEADER(Loginchan); }
#line 5882 "parse_pack.c"
yy836:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy837;
	default:	goto yy83;
	}
yy837:
#line 349 "parse_pack.re"
	{ SET_HEADER(Logintime); }
#line 5893 "parse_pack.c"
yy838:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy913;
	default:	goto yy84;
	}
yy839:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy914;
	default:	goto yy84;
	}
yy840:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy916;
	default:	goto yy84;
	}
yy841:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy918;
	default:	goto yy84;
	}
yy842:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy919;
	default:	goto yy84;
	}
yy843:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy921;
	default:	goto yy84;
	}
yy844:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy922;
	default:	goto yy84;
	}
yy845:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy923;
	default:	goto yy84;
	}
yy846:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy924;
	default:	goto yy84;
	}
yy847:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy925;
	default:	goto yy84;
	}
yy848:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy927;
	default:	goto yy84;
	}
yy849:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy850;
	default:	goto yy83;
	}
yy850:
#line 372 "parse_pack.re"
	{ SET_HEADER(Privilege); }
#line 5981 "parse_pack.c"
yy851:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy852;
	default:	goto yy83;
	}
yy852:
#line 375 "parse_pack.re"
	{ SET_HEADER(RegExpire); }
#line 5992 "parse_pack.c"
yy853:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy854;
	default:	goto yy83;
	}
yy854:
#line 376 "parse_pack.re"
	{ SET_HEADER(RegExpiry); }
#line 6003 "parse_pack.c"
yy855:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy928;
	default:	goto yy84;
	}
yy856:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy929;
	default:	goto yy84;
	}
yy857:
	++cur;
	yych = *cur;
	switch (yych) {
	case ' ':	goto yy857;
	case 'F':
	case 'f':	goto yy931;
	default:	goto yy115;
	}
yy859:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy932;
	default:	goto yy84;
	}
yy860:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy933;
	default:	goto yy84;
	}
yy861:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy934;
	default:	goto yy84;
	}
yy862:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy935;
	default:	goto yy84;
	}
yy863:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy936;
	default:	goto yy84;
	}
yy864:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy937;
	default:	goto yy84;
	}
yy865:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy939;
	default:	goto yy84;
	}
yy866:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy867;
	default:	goto yy83;
	}
yy867:
#line 391 "parse_pack.re"
	{ SET_HEADER(StartTime); }
#line 6086 "parse_pack.c"
yy868:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy940;
	default:	goto yy84;
	}
yy869:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy870;
	default:	goto yy83;
	}
yy870:
#line 399 "parse_pack.re"
	{ SET_HEADER(Uniqueid1); }
#line 6104 "parse_pack.c"
yy871:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy872;
	default:	goto yy83;
	}
yy872:
#line 400 "parse_pack.re"
	{ SET_HEADER(Uniqueid2); }
#line 6115 "parse_pack.c"
yy873:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy874;
	default:	goto yy83;
	}
yy874:
#line 402 "parse_pack.re"
	{ SET_HEADER(UserField); }
#line 6126 "parse_pack.c"
yy875:
	yych = *++cur;
	switch (yych) {
	case 'B':
	case 'b':	goto yy941;
	default:	goto yy84;
	}
yy876:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy942;
	default:	goto yy84;
	}
yy877:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy878;
	default:	goto yy83;
	}
yy878:
#line 275 "parse_pack.re"
	{ SET_HEADER(Address_IP); }
#line 6151 "parse_pack.c"
yy879:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy944;
	default:	goto yy84;
	}
yy880:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy881;
	default:	goto yy83;
	}
yy881:
#line 279 "parse_pack.re"
	{ SET_HEADER(AnswerTime); }
#line 6169 "parse_pack.c"
yy882:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy945;
	default:	goto yy84;
	}
yy883:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy947;
	default:	goto yy84;
	}
yy884:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy948;
	default:	goto yy84;
	}
yy885:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy886;
	default:	goto yy83;
	}
yy886:
#line 286 "parse_pack.re"
	{ SET_HEADER(Bridgetype); }
#line 6201 "parse_pack.c"
yy887:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy950;
	default:	goto yy84;
	}
yy888:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy951;
	default:	goto yy84;
	}
yy889:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy890;
	default:	goto yy83;
	}
yy890:
#line 293 "parse_pack.re"
	{ SET_HEADER(CallsTaken); }
#line 6226 "parse_pack.c"
yy891:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy953;
	default:	goto yy84;
	}
yy892:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy954;
	default:	goto yy84;
	}
yy893:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy956;
	default:	goto yy84;
	}
yy894:
	yych = *++cur;
	switch (yych) {
	case 'G':
	case 'g':	goto yy957;
	default:	goto yy84;
	}
yy895:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy896;
	default:	goto yy83;
	}
yy896:
#line 304 "parse_pack.re"
	{ SET_HEADER(CodecOrder); }
#line 6265 "parse_pack.c"
yy897:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy958;
	default:	goto yy84;
	}
yy898:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy959;
	default:	goto yy84;
	}
yy899:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy960;
	default:	goto yy84;
	}
yy900:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy961;
	default:	goto yy84;
	}
yy901:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy963;
	default:	goto yy84;
	}
yy902:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy903;
	default:	goto yy83;
	}
yy903:
#line 318 "parse_pack.re"
	{ SET_HEADER(DialStatus); }
#line 6311 "parse_pack.c"
yy904:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy905;
	default:	goto yy83;
	}
yy905:
#line 319 "parse_pack.re"
	{ SET_HEADER(Dialstring); }
#line 6322 "parse_pack.c"
yy906:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy964;
	default:	goto yy84;
	}
yy907:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy966;
	default:	goto yy84;
	}
yy908:
	yych = *++cur;
	switch (yych) {
	case 'X':
	case 'x':	goto yy967;
	default:	goto yy84;
	}
yy909:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy968;
	default:	goto yy84;
	}
yy910:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy969;
	default:	goto yy84;
	}
yy911:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy970;
	default:	goto yy84;
	}
yy912:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy971;
	default:	goto yy84;
	}
yy913:
	yych = *++cur;
	switch (yych) {
	case 'X':
	case 'x':	goto yy972;
	default:	goto yy84;
	}
yy914:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy915;
	default:	goto yy83;
	}
yy915:
#line 352 "parse_pack.re"
	{ SET_HEADER(Membership); }
#line 6389 "parse_pack.c"
yy916:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy917;
	default:	goto yy83;
	}
yy917:
#line 355 "parse_pack.re"
	{ SET_HEADER(MOHSuggest); }
#line 6400 "parse_pack.c"
yy918:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy973;
	default:	goto yy84;
	}
yy919:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy920;
	default:	goto yy83;
	}
yy920:
#line 358 "parse_pack.re"
	{ SET_HEADER(ObjectName); }
#line 6418 "parse_pack.c"
yy921:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy975;
	default:	goto yy84;
	}
yy922:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy976;
	default:	goto yy84;
	}
yy923:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy978;
	default:	goto yy84;
	}
yy924:
	yych = *++cur;
	switch (yych) {
	case 'F':
	case 'f':	goto yy979;
	default:	goto yy84;
	}
yy925:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy926;
	default:	goto yy83;
	}
yy926:
#line 367 "parse_pack.re"
	{ SET_HEADER(PeerStatusHdr); }
#line 6457 "parse_pack.c"
yy927:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy980;
	default:	goto yy84;
	}
yy928:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy982;
	default:	goto yy84;
	}
yy929:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy930;
	default:	goto yy83;
	}
yy930:
#line 378 "parse_pack.re"
	{ SET_HEADER(Resolution); }
#line 6482 "parse_pack.c"
yy931:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy983;
	default:	goto yy115;
	}
yy932:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy984;
	default:	goto yy84;
	}
yy933:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy986;
	default:	goto yy84;
	}
yy934:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy987;
	default:	goto yy84;
	}
yy935:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy988;
	default:	goto yy84;
	}
yy936:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy989;
	default:	goto yy84;
	}
yy937:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy938;
	default:	goto yy83;
	}
yy938:
#line 387 "parse_pack.re"
	{ SET_HEADER(SIPLastMsg); }
#line 6535 "parse_pack.c"
yy939:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy990;
	default:	goto yy84;
	}
yy940:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy992;
	default:	goto yy84;
	}
yy941:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy993;
	default:	goto yy84;
	}
yy942:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy943;
	default:	goto yy83;
	}
yy943:
#line 271 "parse_pack.re"
	{ SET_HEADER(AccountCode); }
#line 6567 "parse_pack.c"
yy944:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy994;
	default:	goto yy84;
	}
yy945:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy946;
	default:	goto yy83;
	}
yy946:
#line 281 "parse_pack.re"
	{ SET_HEADER(Application); }
#line 6585 "parse_pack.c"
yy947:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy996;
	default:	goto yy84;
	}
yy948:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy949;
	default:	goto yy83;
	}
yy949:
#line 285 "parse_pack.re"
	{ SET_HEADER(Bridgestate); }
#line 6603 "parse_pack.c"
yy950:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy997;
	default:	goto yy84;
	}
yy951:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy952;
	default:	goto yy83;
	}
yy952:
#line 291 "parse_pack.re"
	{ SET_HEADER(CallerIDNum); }
#line 6621 "parse_pack.c"
yy953:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy999;
	default:	goto yy84;
	}
yy954:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy955;
	default:	goto yy83;
	}
yy955:
#line 301 "parse_pack.re"
	{ SET_HEADER(ChannelType); }
#line 6639 "parse_pack.c"
yy956:
	yych = *++cur;
	switch (yych) {
	case 'Y':
	case 'y':	goto yy1001;
	default:	goto yy84;
	}
yy957:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy1002;
	default:	goto yy84;
	}
yy958:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy1003;
	default:	goto yy84;
	}
yy959:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy1004;
	default:	goto yy84;
	}
yy960:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy1005;
	default:	goto yy84;
	}
yy961:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy962;
	case 'C':
	case 'c':	goto yy1006;
	default:	goto yy83;
	}
yy962:
#line 314 "parse_pack.re"
	{ SET_HEADER(Destination); }
#line 6687 "parse_pack.c"
yy963:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy1007;
	default:	goto yy84;
	}
yy964:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy965;
	default:	goto yy83;
	}
yy965:
#line 321 "parse_pack.re"
	{ SET_HEADER(Disposition); }
#line 6705 "parse_pack.c"
yy966:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy1009;
	default:	goto yy84;
	}
yy967:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy1011;
	default:	goto yy84;
	}
yy968:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy1013;
	default:	goto yy84;
	}
yy969:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy1014;
	default:	goto yy84;
	}
yy970:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy1015;
	default:	goto yy84;
	}
yy971:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy1016;
	default:	goto yy84;
	}
yy972:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy1017;
	default:	goto yy84;
	}
yy973:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy974;
	default:	goto yy83;
	}
yy974:
#line 356 "parse_pack.re"
	{ SET_HEADER(NewMessages); }
#line 6765 "parse_pack.c"
yy975:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy1018;
	default:	goto yy84;
	}
yy976:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy977;
	default:	goto yy83;
	}
yy977:
#line 360 "parse_pack.re"
	{ SET_HEADER(OldMessages); }
#line 6783 "parse_pack.c"
yy978:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy1019;
	default:	goto yy84;
	}
yy979:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy1020;
	default:	goto yy84;
	}
yy980:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy981;
	default:	goto yy83;
	}
yy981:
#line 369 "parse_pack.re"
	{ SET_HEADER(Pickupgroup); }
#line 6808 "parse_pack.c"
yy982:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy1021;
	default:	goto yy84;
	}
yy983:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy1022;
	default:	goto yy115;
	}
yy984:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy985;
	default:	goto yy83;
	}
yy985:
#line 382 "parse_pack.re"
	{ SET_HEADER(SecretExist); }
#line 6833 "parse_pack.c"
yy986:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy1023;
	default:	goto yy84;
	}
yy987:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy1024;
	default:	goto yy84;
	}
yy988:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy1025;
	default:	goto yy84;
	}
yy989:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy1027;
	default:	goto yy84;
	}
yy990:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy991;
	default:	goto yy83;
	}
yy991:
#line 390 "parse_pack.re"
	{ SET_HEADER(SrcUniqueID); }
#line 6872 "parse_pack.c"
yy992:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy1028;
	default:	goto yy84;
	}
yy993:
	yych = *++cur;
	switch (yych) {
	case 'X':
	case 'x':	goto yy1030;
	default:	goto yy84;
	}
yy994:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy995;
	default:	goto yy83;
	}
yy995:
#line 276 "parse_pack.re"
	{ SET_HEADER(Address_Port); }
#line 6897 "parse_pack.c"
yy996:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy1032;
	default:	goto yy84;
	}
yy997:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy998;
	default:	goto yy83;
	}
yy998:
#line 290 "parse_pack.re"
	{ SET_HEADER(CallerIDName); }
#line 6915 "parse_pack.c"
yy999:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1000;
	case 'D':
	case 'd':	goto yy1033;
	default:	goto yy83;
	}
yy1000:
#line 299 "parse_pack.re"
	{ SET_HEADER(ChannelState); }
#line 6928 "parse_pack.c"
yy1001:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy1034;
	default:	goto yy84;
	}
yy1002:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy1035;
	default:	goto yy84;
	}
yy1003:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy1036;
	default:	goto yy84;
	}
yy1004:
	yych = *++cur;
	switch (yych) {
	case '-':	goto yy1037;
	default:	goto yy84;
	}
yy1005:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy1038;
	default:	goto yy84;
	}
yy1006:
	yych = *++cur;
	switch (yych) {
	case 'H':
	case 'h':	goto yy1039;
	case 'O':
	case 'o':	goto yy1040;
	default:	goto yy84;
	}
yy1007:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1008;
	default:	goto yy83;
	}
yy1008:
#line 317 "parse_pack.re"
	{ SET_HEADER(DestUniqueID); }
#line 6982 "parse_pack.c"
yy1009:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1010;
	default:	goto yy83;
	}
yy1010:
#line 330 "parse_pack.re"
	{ SET_HEADER(ExtraChannel); }
#line 6993 "parse_pack.c"
yy1011:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1012;
	default:	goto yy83;
	}
yy1012:
#line 331 "parse_pack.re"
	{ SET_HEADER(ExtraContext); }
#line 7004 "parse_pack.c"
yy1013:
	yych = *++cur;
	switch (yych) {
	case 'Y':
	case 'y':	goto yy1041;
	default:	goto yy84;
	}
yy1014:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy1043;
	default:	goto yy84;
	}
yy1015:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy1045;
	default:	goto yy84;
	}
yy1016:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy1046;
	default:	goto yy84;
	}
yy1017:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy1047;
	default:	goto yy84;
	}
yy1018:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy1048;
	default:	goto yy84;
	}
yy1019:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy1049;
	default:	goto yy84;
	}
yy1020:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy1051;
	default:	goto yy84;
	}
yy1021:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy1052;
	default:	goto yy84;
	}
yy1022:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy1053;
	default:	goto yy115;
	}
yy1023:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy1054;
	default:	goto yy84;
	}
yy1024:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy1055;
	default:	goto yy84;
	}
yy1025:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1026;
	default:	goto yy83;
	}
yy1026:
#line 386 "parse_pack.re"
	{ SET_HEADER(SIP_FromUser); }
#line 7099 "parse_pack.c"
yy1027:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy1056;
	default:	goto yy84;
	}
yy1028:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1029;
	default:	goto yy83;
	}
yy1029:
#line 397 "parse_pack.re"
	{ SET_HEADER(TransferRate); }
#line 7117 "parse_pack.c"
yy1030:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1031;
	default:	goto yy83;
	}
yy1031:
#line 407 "parse_pack.re"
	{ SET_HEADER(VoiceMailbox); }
#line 7128 "parse_pack.c"
yy1032:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy1057;
	default:	goto yy84;
	}
yy1033:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy1058;
	default:	goto yy84;
	}
yy1034:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy1059;
	default:	goto yy84;
	}
yy1035:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy1061;
	default:	goto yy84;
	}
yy1036:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy1062;
	default:	goto yy84;
	}
yy1037:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy1063;
	default:	goto yy84;
	}
yy1038:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy1064;
	default:	goto yy84;
	}
yy1039:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy1065;
	default:	goto yy84;
	}
yy1040:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy1066;
	default:	goto yy84;
	}
yy1041:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1042;
	default:	goto yy83;
	}
yy1042:
#line 332 "parse_pack.re"
	{ SET_HEADER(ExtraPriority); }
#line 7202 "parse_pack.c"
yy1043:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1044;
	default:	goto yy83;
	}
yy1044:
#line 339 "parse_pack.re"
	{ SET_HEADER(Incominglimit); }
#line 7213 "parse_pack.c"
yy1045:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy1067;
	default:	goto yy84;
	}
yy1046:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy1068;
	default:	goto yy84;
	}
yy1047:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy1070;
	default:	goto yy84;
	}
yy1048:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy1072;
	default:	goto yy84;
	}
yy1049:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1050;
	default:	goto yy83;
	}
yy1050:
#line 362 "parse_pack.re"
	{ SET_HEADER(Outgoinglimit); }
#line 7252 "parse_pack.c"
yy1051:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy1074;
	default:	goto yy84;
	}
yy1052:
	yych = *++cur;
	switch (yych) {
	case 'I':
	case 'i':	goto yy1075;
	default:	goto yy84;
	}
yy1053:
	yych = *++cur;
	switch (yych) {
	case 'O':
	case 'o':	goto yy1076;
	default:	goto yy115;
	}
yy1054:
	yych = *++cur;
	switch (yych) {
	case 'U':
	case 'u':	goto yy1077;
	default:	goto yy84;
	}
yy1055:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy1078;
	default:	goto yy84;
	}
yy1056:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy1080;
	default:	goto yy84;
	}
yy1057:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy1082;
	default:	goto yy84;
	}
yy1058:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy1084;
	default:	goto yy84;
	}
yy1059:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1060;
	default:	goto yy83;
	}
yy1060:
#line 302 "parse_pack.re"
	{ SET_HEADER(ChanObjectType); }
#line 7319 "parse_pack.c"
yy1061:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy1085;
	default:	goto yy84;
	}
yy1062:
	yych = *++cur;
	switch (yych) {
	case 'A':
	case 'a':	goto yy1087;
	case 'U':
	case 'u':	goto yy1088;
	default:	goto yy84;
	}
yy1063:
	yych = *++cur;
	switch (yych) {
	case 'P':
	case 'p':	goto yy1089;
	default:	goto yy84;
	}
yy1064:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy1091;
	default:	goto yy84;
	}
yy1065:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy1092;
	default:	goto yy84;
	}
yy1066:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy1093;
	default:	goto yy84;
	}
yy1067:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy1094;
	default:	goto yy84;
	}
yy1068:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1069;
	default:	goto yy83;
	}
yy1069:
#line 346 "parse_pack.re"
	{ SET_HEADER(LocalStationID); }
#line 7381 "parse_pack.c"
yy1070:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1071;
	default:	goto yy83;
	}
yy1071:
#line 351 "parse_pack.re"
	{ SET_HEADER(MD5SecretExist); }
#line 7392 "parse_pack.c"
yy1072:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1073;
	default:	goto yy83;
	}
yy1073:
#line 359 "parse_pack.re"
	{ SET_HEADER(OldAccountCode); }
#line 7403 "parse_pack.c"
yy1074:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy1096;
	default:	goto yy84;
	}
yy1075:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy1097;
	default:	goto yy84;
	}
yy1076:
	yych = *++cur;
	switch (yych) {
	case 'W':
	case 'w':	goto yy1099;
	default:	goto yy115;
	}
yy1077:
	yych = *++cur;
	switch (yych) {
	case 'R':
	case 'r':	goto yy1100;
	default:	goto yy84;
	}
yy1078:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1079;
	default:	goto yy83;
	}
yy1079:
#line 385 "parse_pack.re"
	{ SET_HEADER(SIP_FromDomain); }
#line 7442 "parse_pack.c"
yy1080:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1081;
	default:	goto yy83;
	}
yy1081:
#line 388 "parse_pack.re"
	{ SET_HEADER(SIP_NatSupport); }
#line 7453 "parse_pack.c"
yy1082:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1083;
	default:	goto yy83;
	}
yy1083:
#line 284 "parse_pack.re"
	{ SET_HEADER(BillableSeconds); }
#line 7464 "parse_pack.c"
yy1084:
	yych = *++cur;
	switch (yych) {
	case 'C':
	case 'c':	goto yy1101;
	default:	goto yy84;
	}
yy1085:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1086;
	default:	goto yy83;
	}
yy1086:
#line 303 "parse_pack.re"
	{ SET_HEADER(CID_CallingPres); }
#line 7482 "parse_pack.c"
yy1087:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy1103;
	default:	goto yy84;
	}
yy1088:
	yych = *++cur;
	switch (yych) {
	case 'M':
	case 'm':	goto yy1104;
	default:	goto yy84;
	}
yy1089:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1090;
	default:	goto yy83;
	}
yy1090:
#line 312 "parse_pack.re"
	{ SET_HEADER(Default_addr_IP); }
#line 7507 "parse_pack.c"
yy1091:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy1106;
	default:	goto yy84;
	}
yy1092:
	yych = *++cur;
	switch (yych) {
	case 'N':
	case 'n':	goto yy1108;
	default:	goto yy84;
	}
yy1093:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy1109;
	default:	goto yy84;
	}
yy1094:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1095;
	default:	goto yy83;
	}
yy1095:
#line 341 "parse_pack.re"
	{ SET_HEADER(LastApplication); }
#line 7539 "parse_pack.c"
yy1096:
	yych = *++cur;
	switch (yych) {
	case 'D':
	case 'd':	goto yy1110;
	default:	goto yy84;
	}
yy1097:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1098;
	default:	goto yy83;
	}
yy1098:
#line 377 "parse_pack.re"
	{ SET_HEADER(RemoteStationID); }
#line 7557 "parse_pack.c"
yy1099:
	yych = *++cur;
	switch (yych) {
	case 'S':
	case 's':	goto yy1112;
	default:	goto yy115;
	}
yy1100:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy1113;
	default:	goto yy84;
	}
yy1101:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1102;
	default:	goto yy83;
	}
yy1102:
#line 300 "parse_pack.re"
	{ SET_HEADER(ChannelStateDesc); }
#line 7582 "parse_pack.c"
yy1103:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy1115;
	default:	goto yy84;
	}
yy1104:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1105;
	default:	goto yy83;
	}
yy1105:
#line 308 "parse_pack.re"
	{ SET_HEADER(ConnectedLineNum); }
#line 7600 "parse_pack.c"
yy1106:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1107;
	default:	goto yy83;
	}
yy1107:
#line 313 "parse_pack.re"
	{ SET_HEADER(Default_Username); }
#line 7611 "parse_pack.c"
yy1108:
	yych = *++cur;
	switch (yych) {
	case 'E':
	case 'e':	goto yy1117;
	default:	goto yy84;
	}
yy1109:
	yych = *++cur;
	switch (yych) {
	case 'X':
	case 'x':	goto yy1118;
	default:	goto yy84;
	}
yy1110:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1111;
	default:	goto yy83;
	}
yy1111:
#line 364 "parse_pack.re"
	{ SET_HEADER(PagesTransferred); }
#line 7636 "parse_pack.c"
yy1112:
	yych = *++cur;
	switch (yych) {
	case '\r':	goto yy1119;
	default:	goto yy115;
	}
yy1113:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1114;
	default:	goto yy83;
	}
yy1114:
#line 384 "parse_pack.re"
	{ SET_HEADER(SIP_AuthInsecure); }
#line 7653 "parse_pack.c"
yy1115:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1116;
	default:	goto yy83;
	}
yy1116:
#line 307 "parse_pack.re"
	{ SET_HEADER(ConnectedLineName); }
#line 7664 "parse_pack.c"
yy1117:
	yych = *++cur;
	switch (yych) {
	case 'L':
	case 'l':	goto yy1120;
	default:	goto yy84;
	}
yy1118:
	yych = *++cur;
	switch (yych) {
	case 'T':
	case 't':	goto yy1122;
	default:	goto yy84;
	}
yy1119:
	yych = *++cur;
	switch (yych) {
	case '\n':	goto yy1124;
	default:	goto yy115;
	}
yy1120:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1121;
	default:	goto yy83;
	}
yy1121:
#line 315 "parse_pack.re"
	{ SET_HEADER(DestinationChannel); }
#line 7695 "parse_pack.c"
yy1122:
	++cur;
	switch ((yych = *cur)) {
	case ' ':
	case ':':	goto yy1123;
	default:	goto yy83;
	}
yy1123:
#line 316 "parse_pack.re"
	{ SET_HEADER(DestinationContext); }
#line 7706 "parse_pack.c"
yy1124:
	++cur;
#line 250 "parse_pack.re"
	{
              len = cur - tok;
              tok = cur;
              amipack_type (pack, AMI_RESPONSE);
              amipack_append (pack, Response, "Follows");
              goto yyc_command;
            }
#line 7717 "parse_pack.c"
/* *********************************** */
yyc_value:
	yych = *cur;
	switch (yych) {
	case '\n':	goto yy1131;
	case '\r':	goto yy1133;
	default:	goto yy1129;
	}
yy1128:
#line 417 "parse_pack.re"
	{
              len = cur - tok;
              char *val = substr(tok, len, 0);
              if (hdr_type == HDR_UNKNOWN) {
                amipack_append_unknown (pack, hdr_name, val);
                free (hdr_name);
              } else {
                amipack_append (pack, hdr_type, val);
              }
              free (val);
              goto yyc_value;
            }
#line 7740 "parse_pack.c"
yy1129:
	++cur;
	yych = *cur;
	switch (yych) {
	case '\n':
	case '\r':	goto yy1128;
	default:	goto yy1129;
	}
yy1131:
	++cur;
yy1132:
#line 223 "parse_pack.re"
	{
              if (hdr_name) free (hdr_name);
              amipack_destroy (pack);
              return NULL;
            }
#line 7758 "parse_pack.c"
yy1133:
	yych = *(marker = ++cur);
	switch (yych) {
	case '\n':	goto yy1134;
	default:	goto yy1132;
	}
yy1134:
	yych = *++cur;
	ctxmarker = cur;
	switch (yych) {
	case '\r':	goto yy1136;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':	goto yy1137;
	default:	goto yy1135;
	}
yy1135:
	cur = marker;
	goto yy1132;
yy1136:
	yych = *++cur;
	switch (yych) {
	case '\n':	goto yy1139;
	default:	goto yy1135;
	}
yy1137:
	++cur;
	cur = ctxmarker;
#line 416 "parse_pack.re"
	{ tok = cur - 1; goto yyc_key; }
#line 7838 "parse_pack.c"
yy1139:
	++cur;
#line 228 "parse_pack.re"
	{ goto done; }
#line 7843 "parse_pack.c"
}
#line 442 "parse_pack.re"


done:
  return pack;
}
